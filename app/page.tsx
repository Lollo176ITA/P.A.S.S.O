      ...
      <main className="relative mx-auto flex w-full max-w-6xl flex-col gap-24 pb-24 px-4 sm:px-6 lg:px-10 xl:px-16">
        ...
        <section id="palestra" className="scroll-mt-32">
          <div className="text-center mb-10">
            <p className="text-sm uppercase tracking-[0.3em] text-primary-700 dark:text-primary-300">Cosa faremo</p>
            <h2 className="text-4xl font-bold text-primary-900 dark:text-primary-100">Una palestra di autonomia</h2>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {PRACTICE_TOPICS.map((topic) => (
              <div
                key={topic.title}
                className="rounded-3xl border border-border bg-surface p-6 shadow-lg shadow-primary/5 transition hover:-translate-y-1 hover:border-primary/40"
              >
                <h3 className="text-xl font-semibold text-primary-900 dark:text-primary-100">{topic.title}</h3>
                <p className="mt-3 text-sm text-muted-foreground leading-relaxed">{topic.description}</p>
              </div>
            ))}
          </div>
          <div className="mt-8 text-center">
            <Link
              href="/servizi"
              className="text-primary-700 dark:text-primary-300 hover:text-primary-900 dark:hover:text-primary-100 underline underline-offset-4 transition"
            >
        ...
        <section id="fasce" className="scroll-mt-32">
          <div className="rounded-4xl bg-linear-to-r from-primary-100 via-surface to-primary-50 dark:from-primary-900/40 dark:via-surface dark:to-primary-900/20 p-10 shadow-xl">
            <p className="text-sm uppercase tracking-[0.35em] text-primary-700 dark:text-primary-300 mb-2">Per chi</p>
            <h2 className="text-3xl font-bold text-primary-900 dark:text-primary-100 mb-6">Azioni per fasce d&apos;età</h2>
            <div className="grid gap-6 md:grid-cols-2">
              {AGE_ACTIONS.map((action) => (
                <div key={action.age} className="rounded-2xl border border-border bg-surface/90 p-6 shadow">
                  <h3 className="text-2xl font-bold text-primary-900 dark:text-primary-100">{action.age}</h3>
                  <p className="mt-3 text-sm text-muted-foreground leading-relaxed">{action.description}</p>
                </div>
              ))}
            </div>
          </div>
